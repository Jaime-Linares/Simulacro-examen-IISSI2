...
exports.index = async function (req, res) {
  try {
    const restaurants = await Restaurant.findAll(
      {
        attributes: ['id', 'name', 'description', 'address', 'postalCode', 'url', 'shippingCosts', 'averageServiceMinutes', 'email', 'phone', 'promoted', 'logo', 'heroImage', 'status', 'restaurantCategoryId'],
        include:
      {
...
exports.promote = async function (req, res) {
  ...
  try {
    const restaurantAPromocionar = await Restaurant.findByPk(req.params.restaurantId)
    const restaurantDespromocionar = await Restaurant.findOne({ where: { userId: req.user.id, promoted: true } })
    if (restaurantDespromocionar) {
      await Restaurant.update({ promoted: false }, { where: { id: restaurantDespromocionar.id } }, { transaction })
    }
    await Restaurant.update({ promoted: true }, { where: { id: restaurantAPromocionar.id } }, { transaction })
